...
import { ArtifactParamTypes, ArtifactSubParamTypes, ArtifactNames, ArtifactPositionNames, ArtifactSetNames } from '@/typings/ArtifactMap'
import { __ } from '@/i18n'
...
export default defineComponent({
    ...
    data() {
        ...
        for (let i in SubFilterEquation)
            if (isNaN(Number(i)))
                availableSubFilterEquations.push({ value: SubFilterEquation[i], label: i})
    ...
    <el-dialog :title="title" width="800px" :model-value="show" @update:model-value="$emit('update:show', $event)">
        <article class="artiface-edit-panel">
            <el-select v-model="filter.position" multiple :placeholder="__('位置')">
                <el-option
                v-for="(item, a) in ArtifactPositionNames"
                :key="a"
                :label="__(item)"
                :value="item">
                </el-option>
            ...
            <el-select v-model="filter.set" multiple :placeholder="__('套装')">
                <el-option
                v-for="(item, a) in ArtifactSetNames"
                :key="a"
                :label="__(item)"
                :value="item">
                </el-option>
            ...
            <el-select v-model="filter.main" multiple :placeholder="__('主词条')">
                <el-option
                v-for="(item, a) in ArtifactParamTypes"
                :key="a"
                :label="__(item)"
                :value="item">
                </el-option>
